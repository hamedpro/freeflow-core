...
export function new_transaction_privileges_check(
	...
	if (typeof thing_id === "undefined") {
		...
		function is_thing_meta(thing: any): thing is meta<non_file_meta_value | file_meta_value> {
...
export function calc_cache(transactions: transaction[], snapshot: time_travel_snapshot) {
...
export function flexible_user_finder(
	...
	if (matches_count === 0) {
		...
		var matched_user = tmp.find((item: any) => {
			return (
				[item.thing.value.email_address, item.thing_id].find((i) => i == identifier) !==
				undefined
			);
		});
...
export function finder(
	...
	return cache
		.filter((ci: cache_item<any>) => {
			...
			if (parsed_finder_query.includes("write-access")) {
				if (ci.thing.type !== "meta") {
					...
					if (ci.thing.value.modify_thing_privileges !== user_id) return false;
				}
			}
		...
		.sort((ci1, ci2) => {
			var ci1_creation_time = thing_transactions(transactions, ci1.thing_id)[0]?.time;
			var ci2_creation_time = thing_transactions(transactions, ci2.thing_id)[0]?.time;

			var ci1_update_time = thing_transactions(transactions, ci1.thing_id).at(-1)?.time;
			var ci2_update_time = thing_transactions(transactions, ci2.thing_id).at(-1)?.time;
