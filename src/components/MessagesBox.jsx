	...
	const edit_message_handler = async (message_id) => {
		await uhc.request_new_transaction({
			new_thing_creator: (prev) => ({
				...prev,
				value: { ...prev.value, text: window.prompt("enter new text for this message :") },
			}),
			thing_id: message_id,
		});
	};
	async function new_message() {
		...
		var new_meta_id = await uhc.request_new_transaction({
			new_thing_creator: () => ({
				type: "meta",
				value: {
					thing_privileges: uhc.find_thing_meta(thing_id).thing.value.thing_privileges,
					locks: [],
				...
				{messages_to_show.map((cache_item) => (
					<div
						...
						className={`relative w-full flex ${
							uhc.find_first_transaction(cache_item.thing_id).user_id === uhc.user_id
								? "justify-start"
								: "justify-end"
						}`}
					>
						...
						>
							<p className="mb-2">{cache_item.thing.value.text}</p>
							<StyledDiv
