...
export const Task = ({ cache_item }) => {
	...
	async function toggle_step_state(step_index) {
		var tmp = uhc.find_thing_meta(cache_item.thing_id).thing.value.thing_privileges.write;
		if (tmp !== "*" && !tmp.includes(uhc.user_id)) {
			alert("you have not write access to this thing");
			return;
		}
			...
			new_thing_creator: (prev) => ({
				...
				value: {
					...
					steps: prev.value.steps.map((step, index) =>
						index === step_index ? { ...step, is_done: !step.is_done } : step
					),
				},
			}),
						...
						<tbody>
							{cache_item.thing.value.steps.map((step, index) => (
								<tr key={index}>
									...
									<td>{step.percent}</td>
									<td onClick={() => toggle_step_state(index)}>
										{step.is_done === true ? (
