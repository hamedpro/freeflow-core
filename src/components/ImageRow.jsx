...
export function ImageRow({ items = [], title, images = [], className = "", onClick = () => {} }) {
	return (
		<div
			onClick={onClick}
			className={[
				"hover:bg-blue-100 duration-300 cursor-pointer w-full border border-stone-500 rounded-lg flex overflow-hidden mb-2",
				className,
			].join(" ")}
		>
			<div className="w-1/3 h-56 py-2 bg-blue-400 flex justify-center items-center">
				<img src={images[0]} style={{ height: "100%", objectFit: "contain" }} />
			</div>
